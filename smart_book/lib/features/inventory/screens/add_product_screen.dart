...
class _AddProductScreenState extends State<AddProductScreen> {
  ...
  String _getCurrentActivityType(BuildContext context) {
    return context.read<SystemConfigurationCubit>().state.settings.activeBusinessModule.name;
  }
    ...
    return BlocListener<AddProductCubit, AddProductState>(
      listener: (context, state) {
        if (state is AddProductLoading) {
          ...
          if (Navigator.canPop(context)) Navigator.pop(context);
          _showSnackBar(context, context.lang.saveSuccess, Colors.green);
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(context.lang.addProduct, style: const TextStyle(fontWeight: FontWeight.bold)),
          centerTitle: true,
        ),
        body: Form(
          key: _formKey,
          child: ListView(
            ...
            children: [
              SectionHeader(title: context.lang.basicInfo, icon: Icons.inventory_2),
              BlocBuilder<AddProductCubit, AddProductState>(
                buildWhen: (previous, current) => previous.units != current.units,
                builder: (context, state) {
                  final units = state.units;
                  final baseUnitName = units.isNotEmpty && units.first.unitName.trim().isNotEmpty
              ...
              InventoryExtensionManager.getExtensionWidget(
                ...
                onIsIngredientChanged: (value) {
                  setState(() => _isIngredient = value ?? false);
                },
              ),
        ...
        bottomSheet: ProductSaveBottomSheet(
          onSavePressed: () => _onSave(context, context.read<AddProductCubit>().state),
        ),
      ),
    );
  }
